...
# Compatible with the OTTO spec used across your SDKs.
...
class Otto:
    ...
    def _init_context_from_header(self, header:bytes, raw_key:bytes)->Tuple[bytes, bytes, bytes, bytes]:
        ...
        kdf_id = header[6]
        if kdf_id != KDF_RAW: raise ValueError("this helper expects RAW key mode header")
    ...
    def encrypt_file(self, in_path:str, out_path:str, raw_key:bytes):
        ...
        with open(in_path, "rb") as fin, open(out_path, "wb") as fout:
            ...
            while True:
                ...
                ct_tag = aes.encrypt(nonce, chunk, aad)  # ciphertext || tag(16)
