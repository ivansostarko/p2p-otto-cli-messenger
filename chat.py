...
import base64, os, socket, struct, threading, json, sys, tempfile, pathlib, time
...
class Session:
    def __init__(self, nickname:str):
        ...
        self.pub = bytes(self.priv.public_key)  # 32 bytes
        self.peer_nick = None
        self.peer_pub = None
    ...
    def derive_after_peer(self, peer_pub: bytes):
        ...
        # salt binds both pubs deterministically (order-independent)
...
def run_host(nickname:str, listen_port:int):
    ...
    # receive handshake first (client sends first), then respond
...
def chat_loop(conn: socket.socket, sess: Session):
    ...
    while True:
        ...
        if msg.startswith("/file "):
            ...
            print(f"[i] Sent file '{fname}' ({len(data)} bytes encrypted)")
            continue
        # normal text
